...
public class SSOConfig {
	...
	private static com.baomidou.kisso.SSOConfig SSO_CONFIG = null;
	 ...
	 * new 当前对象
	 */
	public static com.baomidou.kisso.SSOConfig getInstance() {
		if (SSO_CONFIG == null) {
			SSO_CONFIG = new com.baomidou.kisso.SSOConfig();
		}
		return SSO_CONFIG;
	}
	 ...
	 * 设置初始化（可动态修改配置内容）
	 */
	public synchronized static void setInstance(com.baomidou.kisso.SSOConfig ssoConfig) {
		if (ssoConfig == null) {
	...
	public synchronized void initProperties(Properties props) {
		if (props != null) {
			setInstance(new com.baomidou.kisso.SSOConfig(new SSOProperties(props, SSO_RUN_MODE, this.getRunMode())));
		} else {
